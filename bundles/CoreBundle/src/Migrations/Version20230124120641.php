...
final class Version20230124120641 extends AbstractMigration
{
    ...
    {
        if ($schema->hasTable('classificationstore_keys') && $schema->getTable('classificationstore_keys')->hasColumn('definition') === true) {
            $this->addSql('ALTER TABLE `classificationstore_keys` MODIFY COLUMN `definition` json;');
        }
        if ($schema->hasTable('users') && $schema->getTable('users')->hasColumn('keyBindings') === true) {
            $this->addSql('ALTER TABLE `users` MODIFY COLUMN `keyBindings` json;');
        }
        if ($schema->hasTable('gridconfigs') && $schema->getTable('gridconfigs')->hasColumn('config') === true) {
            $this->addSql('ALTER TABLE `gridconfigs` MODIFY COLUMN `config` json;');
        }
        if ($schema->hasTable('importconfigs') && $schema->getTable('importconfigs')->hasColumn('config') === true) {
            $this->addSql('ALTER TABLE `importconfigs` MODIFY COLUMN `config` json;');
        }
        if ($schema->hasTable('targeting_storage') && $schema->getTable('targeting_storage')->hasColumn('value') === true) {
